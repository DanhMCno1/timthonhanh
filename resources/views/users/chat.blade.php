    ...
    <div class="flex flex-row h-full w-full bg-[#F7F9FC] gap-2">
        <div class="flex flex-col flex-auto h-full bg-white shadow rounded-b-lg">
            <div class="relative flex flex-col flex-auto flex-shrink-0 h-full">
                <div class="h-16 flex flex-row justify-start items-center p-2 px-5 shadow-md gap-2">
                    <span class="inline-flex overflow-hidden items-center justify-center size-10 rounded-full bg-gray-500 font-semibold text-white leading-none">
                        <img class="w-full h-full" src="{{ Storage::url( $modelsRequest->staff->image->path ) }}">
                    </span>
                    {{ $modelsRequest->staff->name }}
                </div>
                ...
                <form id="send-message" class="flex flex-row items-end rounded-xl bg-white w-full p-2 gap-x-2" action="{{ route('messages.store', $modelsRequest) }}" method="POST">
                    @csrf
...
<script type="module">
    $(document).ready(function() {
        ...
        function loadMessages(scroll = false) {
            ...
            return axios.get('{{ route('messages.index', $modelsRequest) }}', {
                params: {
                    quantity: quantity
                }
            })
        ...
        sendForm.on('submit', function (e) {
            ...
            axios.post('{{ route('messages.store', $modelsRequest) }}', formData)
                .then(function () {
                    quantity += 1;
                    loading = false;
                    loadMessages(true);
